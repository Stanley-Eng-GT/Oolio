...
class ProductRepository {
  ...
  Future<Result<List<ProductModel>, Exception>> getProductData() async {
    try {
      final result = await _httpManager.request(
          requestEntity: RequestEntity(path: EndPoints.product));

      final Result<List<ProductModel>, Exception> value = switch (result) {
        Success(value: final data) => Success(
